...
![社員一覧・管理者](docs/screenshots/02-list-admin.png)

検索・絞り込み・CSV出力・ページング。行ごとに「編集」「退職」「削除」。
E0007 はすでに退職済みなので「退職」ボタンを出していない。

### 同じ画面を、閲覧のみの利用者が開いたとき

![社員一覧・閲覧のみ](docs/screenshots/03-list-viewer.png)

「新規登録」ボタンと「操作」列が消える。検索とCSV出力は残る。

### 権限のない操作は、URLを直接叩いても止まる

![権限不足](docs/screenshots/04-forbidden.png)

ボタンを隠すのは親切のためで、防御ではない。
閲覧のみの利用者が `/employees/new` を直接開いても
`SecurityConfig` が 403 を返すことを、テストで固定してある。

### 同時編集の競合検知

![競合検知](docs/screenshots/05-conflict.png)

2人が同じ社員を開いて、後の人が保存したときに先の人の変更を黙って消さない。
`@Version` による楽観ロック。止めるだけでなく最新の内容を読み直して表示するので、
利用者は相手の変更を見てからやり直せる。

### 部署別人数

![部署別人数](docs/screenshots/06-summary.png)

在籍・休職・退職を分けて数える。
該当者が1人もいない部署も 0 名として残す（行が消えると集計漏れと区別できないため）。

### 入力チェック

![入力チェック](docs/screenshots/07-validation.png)

エラーは項目ごとにその場へ出す。入力済みの内容は消さない。

### ログイン

![ログイン](docs/screenshots/01-login.png)

開発プロファイルのときだけ、画面下に確認用アカウントを表示する。本番では出ない。
